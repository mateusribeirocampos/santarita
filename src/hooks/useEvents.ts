...
export function useEvents(params?: {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      const { upcomingEvents } = await import('../constants/upcomingEvents');
      const mockEvents: Event[] = upcomingEvents.map(event => ({
        ...event,
        id: event.id.toString()
      }));
      console.log('⚠️ [useEvents] Usando dados mock devido ao erro:', mockEvents);
